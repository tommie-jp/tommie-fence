...
> **本プロジェクトは現在、αバージョンです。** およそ動きますが、細かい動きが
> まだ不安定です。[プレイグラウンド](https://tommie-jp.github.io/tommie-fence/)ですぐ
> お試しできます。
...
**3 つとも描ける** — 回路図も、TeX を WebAssembly で走らせて同じ図を出す
(資材を落とすのは circuit を初めて描くときだけ)。**図を掴んで動かすマップも
開く** — 部品をドラッグするとフェンスの番地が書き換わる、拡張と同じものが
動く。例は各パッケージの `examples/` の `.md` がそのまま 47 本。
