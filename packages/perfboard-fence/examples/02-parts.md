...
下は **50Ω 系の 10 dB T 型アッテネータ** — 直列に 27Ω が 2 本、分岐に 36Ω。
測定器の前に挟んで電力を 1/10 (電圧で約 1/3) に落とす。理屈どおりは 26.0Ω と
35.1Ω だが、**E24 に載っていて手に入りやすい 27Ω と 36Ω に寄せてある**
(この値で 10.1 dB、入力 51.5Ω)。どちらも 2 桁で書ける値なので帯は 4 本 —
`27` は赤・紫・黒に、許容差の茶が付く。
...
points:
  ...
  GND: f1
parts:
  R1: resistor c3 c6 27
  R2: resistor c9 c12 27
  R3: resistor c7 f7 36
wires:
  - IN -- a3
  - a3 -- c3
  - c6 -- c7
...
**この 10 dB アッテネータは図04・図05 にもう一度出てくる。**中身は同じで、
信号の入口だけを SMA コネクタに替えた図になる。**帯の本数を選ぶ書き方は無く、
書いた値が決める** — 精度が要って `26r1` (26.1Ω、E96) と書けば、3 桁ぶんの
5 帯になる。
...
中身は図02 と同じ 10 dB アッテネータ。**入口を SMA に替えただけ**で、抵抗の値も
並びも動かしていない。アースは板の縁を回して両方のコネクタと分岐をまとめる。
...
parts:
  ...
  R2: resistor e8 e11 27
  R3: resistor e7 h7 36
wires:
  - c3 -- e3
  - e6 -- e7
  - e7 -- e8
...
parts:
  ...
  R1: resistor e3 e6 27
  R2: resistor e11 e14 27
  R3: resistor e8 h8 36
wires:
  - e1 -- e3
  - e6 -- e8
  - e8 -- e11
...
中身は図02・図04 と同じ **50Ω 系の 10 dB T 型アッテネータ** (直列 27Ω が
2 本、分岐に 36Ω)。**違うのは入口だけ** — 縦置きの SMA を端面実装に替えると、
同じ回路がこの姿になる。測定器の前に挟んで反射を抑える、板 1 枚で作れるものの代表。
