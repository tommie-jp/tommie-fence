...
**載せる辺は番地が決める** — 縁側の GND を左の銅箔 (`c0`) に書けば左向き、
右の銅箔 (`f17`) に書けば右向きに出る。
図は実物の外形図と同じ 3 段 — 左から**ねじ部** (1/4-36UNS の筋)、**ねじなし**、
**台座**。**台座の右端が板の縁**に来る (実物もそこで板を挟む。台座の厚さは 1mm)。
そこから先は**凹の形**で、**上下にアース**が伸び、その口の中の**白い接点**が
アースの足になる。**中心導体はその先から**出て、さらに内側の穴に入る。
...
points:
  GND: h1
parts:
  J1: sma/female-edge e1 e0
  J2: sma/female-edge e16 e17
...
上の図の GND は **`e0`** と **`e17`** — どちらも板の外の番地で、**縁の銅箔
(スロット) の位置**にあたる。銅箔は穴の格子のちょうど 1 つ外に並んでいるので、
`0` 列と `17` 列がそのままその銅箔になる。つまり**脚を銅箔に半田付けした**
ことになり、ネットにもそう出る。
...
**アースの穴は埋まらない。** 端面実装の脚は穴に挿さらず、板の縁を凹の腕で
挟んで**その上で半田付けする**ので、半田で埋まった穴ではなく**凹の中の白い
接点**が付ける場所になる。配線もそこへ届く。

**その脚も配線でつなぐ。** 実物の銅箔は 1 枚続きなので脚どうしはつながって
いるが、**この板では穴も銅箔も 1 つずつが独立**していて、図の上で結ばれて
いないものは組む人にも結ばれていない。線を引いておけば、どこを半田付けするのかが
図に出る。
