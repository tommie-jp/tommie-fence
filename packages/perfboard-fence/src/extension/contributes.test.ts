...
describe('extension manifest', () => {
  ...
  test('contributes a preview stylesheet so the extension folder is a resource root', () => {
    expect(manifest.contributes['markdown.previewStyles']).toEqual(['./media/perfboard.css']);
  });
  ...
  test('names the classes the stylesheet defines', () => {
    const css = readText('media/perfboard.css');

    for (const selector of ['.perfboard', '.perfboard-error-card', '.perfboard-errors', '.perfboard-notice']) {
