...
**縦に置いた部品は、キャプションも縦になる** (時計回りに 90 度)。横のままだと、
細長い部品の脇に長い字が伸びて隣の部品や配線に被る。**図ごと回すのではなく**
字の並びだけを回すので、読む向きは左から右のまま。
...
`n5` (板の下) のように書く ([番地](#番地))。箱の**左上がその番地**に来て、足は板の
側へ出る。帯に並べると置きたかった場所と関係なく散るので、**並べ方を自分で決めたい
ときはこちら**。板の寸法には機器のはみ出すぶんも入るので、題や書き出しと重ならない。
