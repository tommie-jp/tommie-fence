...
| --- | --- | --- |
| `type` | `device` (**必ず書く**) | — |
| `at` | `top` か `bottom` (板のどちら側の帯に置くか) | `top` |
...
**機器へつなぐ配線は図に線を引かない。** 板の上に線が出ると、そこに挿す場所が
あるように見えてしまう。導通だけがネットに効く。色を書いても図には出ないので、
書いたときはその旨のお知らせが出る (黙って消すと、効かない指定を書き続けることになる)。
...
`debug` と `stamp` は `on` / `off` で書く (`true` / `false` でも同じ)。
...
| 機器へつなぐ配線に書いた色 | 線を引かないので図に出ない。黙って消すと効かない指定を書き続ける |
