...
**`text` だけは字を `:` の後ろに書く。** YAML のプレーンスカラーにはコロンと
空白の並びを書けないので、`- text c3 "R1: resistor …"` は黙ってマップとして
読まれ、エラーにもならずに消える。値の側に置けば、引用が要るかどうかを
YAML が決める。**3 つのフェンスで同じ形**。

**番地のあとに書けるのは語だけ** (色と向き。順不同)。字と語が `:` で分かれた
ので、`text` にも色を書けるようになった。
...
読めた所まで組んで返す (`board:` が無いときは 25x15 で描き、要ることを帯で言う)。
図が消えると、エディタで部品を置くことも動かすこともできなくなるため。
