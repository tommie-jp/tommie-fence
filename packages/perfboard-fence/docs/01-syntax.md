...
**横置きは 3 本足で書く。** `J1: sma/female-edge c1 b0 d0` — **中心導体を先に、
凹の両端の先端をあとに**。実物の凹は板の縁を上下から挟んでいて、半田付けするのは
腕の先端の 2 点、つまり**中心線の上下の行の縁の銅箔**に来る (上の例なら `c` 行の
中心導体に対して `b` 行と `d` 行)。凹は 1 つの金物なので、**2 本の先端は配線を
引かなくても同じネット**に乗る — ジャンパはどちらか片方へ付ければよい。
載せる辺は番地が決める (列が違えば左右の縁、同じなら上下の縁)。胴は板の外へ
出るぶん**画布が広がる** (切って描くと、図を見た人は切れたことに気づけない)。
