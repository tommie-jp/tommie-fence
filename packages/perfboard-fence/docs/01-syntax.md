...
大きさは**列 × 行**で書く。板が「72×47.5mm」と長辺 × 短辺で売られているのと
同じ順。
...
**名前の付いた板 (`akizuki-c` など) はまだ無い。** 秋月の商品ページは寸法と
ピッチしか書いておらず、穴数の一次情報がどこにも無いため。実物で数えるまでは、
書く人が自分の板を見て書く。
...
![図02 26 行を超える板](out/01-syntax-2.svg)

## 題 (`title:`)

図の上に出る題。文章から「図02 を直して」と指せるようにするためのもの。
...
![図03 部品の置き方](out/01-syntax-3.svg)
...
![図04 points で名前を付ける](out/01-syntax-4.svg)
