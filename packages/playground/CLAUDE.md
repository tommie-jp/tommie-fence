...
15. **開く口は 3 つ、書き戻す口は 1 つ** (`files.ts`。52 の docs/43)。
    釦 (`<input type="file">`)・落とす・`?doc=` のどれも `openDoc` へ来る。
    書き戻すのは**同じ名前でダウンロード** — 頁からその場のファイルへ書く道は
    ブラウザによっては無い。**改行の形は開いたときのものを保つ**
    (テキスト欄が値を LF に均すので、Windows の `.md` を開いて保存しただけで
    全行が変更扱いになる)。**直したまま閉じさせない** (`beforeunload`)。
