...
16. **出来事は `say()` を通す** (52 の docs/45)。帯 (2 秒で消える) と
    **ログ** (最後の 20 行) の両方に出る。別の口を作ると、呼び忘れた出来事
    だけがログに出ない。マップの帯へ出た一言も `onStatus` でログへ落とす。
    ログは読み込み直すと消える (文書は外のファイルなので、そちらを見れば
    分かる)。
17. **GUI で編集しているあいだは、フェンスの字を出さない** (`body.editing`)。
    同じ字が 2 か所にあると、どちらが正か迷う。**図は残す** — マップの図は
    掴むための姿で、出来上がりは右の図が正 (circuit はとくに違う)。
