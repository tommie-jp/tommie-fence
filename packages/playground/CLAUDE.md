...
16. **URL は文書ではない** (52 の docs/43)。頁は `#` に図を書かない。
    書く側 (`encodeShare`・「リンクをコピー」・QR・短縮の表引き) は落とした。
    **読む側は残す** — 配ってあるリンク (`#<種類>/<base64url>`) と、それを
    指している s.tommie.jp の転送ページ 109 本を切らないため (約束 8、docs/37)。
    図を渡す口が要るときは、**文書の形で**作り直す (`.md` を渡す)。
...
- `src/files.ts` — 開く・書き戻すの決め事 (改行の形、名前、`?doc=` の綴り)
- `src/demo.ts` — 「試す」釦の中身
