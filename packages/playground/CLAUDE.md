...
14. **短縮リンクは引くだけ。作らない** (`shorts.ts`。判断は 52 の docs/42)。
    QR に入れる URL は、s.tommie.jp の `links.json` に載っていれば短いほうに
    差し替える。**表の正は向こう** (`links.tsv`) で、こちらは行き先の URL で
    引くだけ。短縮を作るには転送ページを置く必要があり、**この頁はサーバを
    持たない** — 手で直したフェンスは長いままになる。
    表を取りに行くのは**QR を開いたときの 1 度だけ**。取れなければ長い URL の
    まま出す。例の分は `scripts/shortLinks.mjs` が TSV を出し、向こうの
    `./doShort.sh --merge` が流し込む。**例を直したら流し込み直す。**
