...
export type FenceView = {
  /** 升目 (エスケープ済み)。 */
  readonly map: string;
  /** 読めなかったところとお知らせの帯。無ければ空。**ERC は入らない** (下の `erc`)。 */
  readonly issues: string;
  /**
   * ERC — そのとおりに組んでも動かないところ。**帯の「検査 N」の釦の向こう**に
   * 畳んでおき、押したときだけ広げる (52 の docs/55)。
   *
   * **持たないフェンスもある。** breadboard は列が最初から導通していて
   * 繋ぎ忘れが目に留まるので ERC を持たない。持たなければ殻は釦を組まない —
   * 数えるものが無いのに常に 0 の釦は、狭い帯のノイズにしかならない
   * (`wireColors?` / `fineFor?` と同じ、任意の口で能力を表す形)。
   */
  readonly erc?: {
    /** 件数。**畳んでいても数える** — 釦に出す数字がそれ。 */
    readonly count: number;
    /** 広げたときに出す行 (組み上がった HTML)。 */
