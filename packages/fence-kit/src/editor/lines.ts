...
/**
 * 同じ書き方に**足せない**ときの文面。消す側と別に持つのは、
 * 綴りを差し替えて作ると尻尾が食い違うため (「行ごと足せません。手で消します」に
 * なっていた)。行が 1 つのものに対応しないのは消すときと同じ理由。
 */
export const FLOW_ADD_REFUSAL = 'フロー形式 (1 行に書いた形) には行を足せません。手で書きます';

/**
 * 行の中を書き換えると**読み方が変わる**ときの文面。範囲が決まらない (項目が
 * 次の行へ続く) か、書いたあと読み直すと狙った中身にならなかったとき。
 * 壊した字を書くより断る。3 つのフェンスで同じ文面にする。
 */
/**
 * 1 行に並べた項目のどれを掴んだか**見分けられない**ときの文面。注釈と配線の名札は
 * 行番号なので、2 つ目を掴んでも 1 つ目を書き換えてしまう。
 */
