...
describe('extractBreadboardFences', () => {
  ...
  test('finds the short spelling ```bread as well as the long one', () => {
    const markdown = ['```bread', 'board: half', '```', '```breadboard', 'board: half', '```'].join('\n');

    expect(extractBreadboardFences(markdown).map((one) => one.line)).toEqual([1, 4]);
  });
