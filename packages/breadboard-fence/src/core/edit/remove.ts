 ...
 */
const notePoints = (note: NoteSpec, id: string): boolean => note.targets.includes(id);
...
export function deletePart(source: string, id: string): RemovalResult {
  ...
  const noteLines = new Set(doc.notes.filter((note) => notePoints(note, part.id)).map((note) => note.line));
