...
describe('1 行に並べたものの改名', () => {
  test('同じ行の注釈 2 つが指す名前を両方書き換える', () => {
    const source = `${HEAD}notes: [circle R1 red, circle R1 blue]\n`;
    const result = editor.rename(source, 'R1', 'R9');
    expect(result.ok && applyRewrite(source, result.value)).toBe(`${HEAD.replace('R1:', 'R9:')}notes: [circle R9 red, circle R9 blue]\n`);
  });
