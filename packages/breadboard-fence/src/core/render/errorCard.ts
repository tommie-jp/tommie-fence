...
const renderLines = (errors: readonly FenceError[], x: number, y: number, palette: Palette): string =>
  lines(errors)
    .map((line, index) =>
...
export function renderErrorBanner(
  ...
  return plate + renderLines(errors, x + PADDING, y + PADDING + 4, palette);
}
...
export function renderErrorCard(errors: readonly FenceError[]): string {
  ...
  return [
    ...
    title,
    renderLines(errors, PADDING * 2, PADDING + LINE_HEIGHT * 2 + 4, palette),
    '</svg>',
  ].join('\n');
}
