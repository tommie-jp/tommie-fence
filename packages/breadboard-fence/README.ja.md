...
code --install-extension packages/breadboard-fence/breadboard-fence-0.5.0.vsix --force
```
...
図を見ながら部品や穴を掴んで動かせる。**位置を穴番地で書く**フェンスなので、
「動かす」は行の中の綴りを差し替えるだけで済む。

- `.md` のタブの頭の一覧から **`breadboard Editor`** を選ぶと、そのタブが
  図のエディタになる。横に開くなら
  `Breadboard Fence: 図を掴んで動かす (マップ)`
- **掴むのは図そのもの** — 別の升目ではなく、穴の上に透明な当たり判定が
  重なっているだけ。見えている絵と掴む場所が食い違わない
- **操作は KiCad 流で、3 つのフェンスで同じ** — 鍵はカーソルの下の物に効き、
  `M` で持ち上げてクリックで置く (ドラッグでも可)、`A` で部品、`W` で配線、
  `G` で節点を引きずる。右に道具の列、左に属性、下に状態行。詳しくは fence-kit の
...
- **置く・消す・つなぐ・回す・名前や値を直す**までできる。`A` で種類を選ぶと
  ...
  **回す (`R`) と反転する (`X`) は掴む物によって行の直り方が変わる** —
  足を並べて書く部品は番地が動き、アンカー 1 つで置く形は向きの語と行が直る
- **動かす前に止めない。** 前後でネットリストを比べ、離れた接続とつながった
  接続を**動かしたあとに**帯へ出す。同じ列の 5 穴はつながっているので、
  同じ導通の中で寄っただけなら何も言わない
- 読めなかった行と ERC のお知らせも同じ帯に出て、**押すとその行へ飛ぶ**
- **YAML は組み直さない。** 手書きのコメントと整形はそのまま残る

タクトスイッチを回す・1 列ヘッダを裏返すのはできない (対称なので、
やっても同じ図になる)。詳しくは
[docs/03-図を掴んで動かす.md](docs/03-図を掴んで動かす.md)。
