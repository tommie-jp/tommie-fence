...
- ボード外の機器 (`device`) が箱に出すのは **`label` (書かなければ ID)** だけ。
  `value` は図のどこにも出ないので、書くと行番号つきで報告して図からは落とす。
- `at:` は**機器を上下どちらの帯に置くか**の指定。板に挿す部品の位置は `holes` で
  決まるので、機器以外に `at:` を書いたときも同じように報告して落とす。
...
- 載るのは**図に描けた部品だけ**。読めなかった部品は図の下のエラー帯にだけ出る。
...
5 行目: 部品 BAT: 機器 (device) に value は使いません。箱に出す名前は label に書きます
3 行目: 部品 R1: at は機器 (device) にだけ使います。板に挿す部品の位置は holes で決まります
```

最後の 2 つは**図が描けたうえで出る**帯。読めた部分 (部品・ピン・配線) はそのまま描き、
使われなかった指定だけを落として理由を言う。
`style` の範囲外の数値を端に寄せて報告するのと同じ立て付け。
