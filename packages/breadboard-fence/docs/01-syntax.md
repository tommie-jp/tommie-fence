...
- 配線の色: red, black, white, gray (grey も可), orange, yellow, green, blue,
  purple, brown, pink。**色を書かなければ gray**。
  板と同じ明るさの `white` と、部品の足と見分けにくい既定の gray は、
  色はそのままに**濃い灰色で縁取って**描く (ほかの色は縁取らない)。
...
- **極性・向きのある 2 端子は、先に書いた穴が + 側 (アノード)** →
  [極性と向き](#極性と向き)。
- `transistor` は TO-92 の**上から見た D 形**で描く (平らな面はピン名の側)。
  **どの足が何かは形では示さない** — 足の並びは品種ごとに違うため、
  どの穴がどの足かはピン名で示す。
...
- DIP は**胴の左端の列の穴**を 1 つ書く (溝の上の `e` 行でも下の `f` 行でも同じ)。
  足は**実物を上から見た並び** — 切り欠きを左にして 1 番が左下 (`f` 行)、
  ピン 1〜N/2 が f 行を左から右へ、残りが e 行を右から左へ戻る (反時計回り)。
  ピン 1 と ピン N は実物と同じく溝をはさんで隣り合う。`r180` で 1 番が右上へ移る。
  0.12.0 までは `e` 行に書くと 1 番が左上に来ていた (実物の鏡像)
- **DIP と 1 列ヘッダのラベルは、本体の枠に収まるまで字を縮めて描く。**
  長い品番や日本語のラベルを書いても枠から出ない (板の上のキャプションのように
  `…` で切ることはしない。品番は最後まで読めるほうが実物と突き合わせやすい)。
- コンデンサの値は**胴の 3 桁コード**として刷られる (`100n` なら `104`)。
  刷るのは無極性のもの (セラミック・フィルム) だけ — 実物の電解とタンタルは
  容量をそのまま刷る。
- 抵抗の値 (`10k` `4k7` `1R` `2.2M`) はカラーコードの帯として描かれる。
  **帯の本数は桁数で決まる** (2 桁なら 4 帯、3 桁要るなら 5 帯)。許容差は
  値のうしろに書け (`10k 5%`)、書かなければ ±1% (金属皮膜の標準)。
  温度係数を足すと 6 帯 (`10k 1% 50ppm`)。
...
style:
  ...
  check: off         # on (既定) / off。図の中身の検査を掛けるか
parts:
  R1: resistor a5 a10 330
  D1: led b12(A) b13(K) red
wires:
...
- `debug: off` で伏せられるのは**お知らせだけ**。読めなかった行は伏せられない
  → [エラーとお知らせの出方](#エラーとお知らせの出方)。
- **`check: off` は `debug: off` とは違う。** あちらは「言うのをやめる」、
  こちらは**「見るのをやめる」**。外れるのは**読めているものへの検査**
  (足と配線が同じ穴に来ている、など) だけで、**読めなかった行は必ず出る**。
  まだ描きかけの図や、実物にする気の無い説明図で外す。
- `stamp: on` は右下に `breadboard-fence 0.13.1` と小さく刻む。**字は選べない**
...
- **端点が部品の足と同じ穴なら、部品のほうが同じ列の空いている行へ寄って描かれる**
  ...
  **2 本足どうしの胴が同じ行で重なる**ときも、後に書いたほうを同じように寄せ、
  こちらは書いたとおりに組めないのでお知らせで言う。
...
- **`text` だけは字を `:` の後ろに書く。** 番地のうしろに引用で字を書く
  (`- text a5 "R1: resistor …"`) と、YAML がコロンで切るので字の頭 (`"R1`) が語として
  読まれ、「注釈の知らない語です」と言われる。字の中にコロンと空白の並びがあるときは、
  `:` の後ろの字を引用で囲む (`- text a5: "R1: resistor a1"`)。囲まないと YAML の
  構文エラーになる。

- **`text` には向きを書ける** (`- text d4 r90 red: ここから電源`)。語は部品と
  同じ `r90` / `r180` / `r270` / `mirror` で、向きを書けるのは `text` だけ。
  **回るのは指し先のまわり** — 字の真ん中で回すと、指した所から離れていく。
  **`mirror` は字を裏返さない**。鏡文字は読めないので、**指し先の反対側へ移す**
  の意味にしてある (上に何かあって重なるときに下へ逃がすためのもの)。
...
parts:
  ...
  X1: crystal/hc49 c19 c21 12.8M
  X2: crystal/cylinder c26 c28 32.768k
...
- `@` に書くのは**基板の左端の列の穴**だけ (DIP と同じ)。基板の幅は 0.7 インチ (穴 7 つぶん) なので、
  ピンの 2 列は上下ブロックの**同じ位置の行** (`a`↔`f`、`b`↔`g`、`c`↔`h`、`d`↔`i`、
  `e`↔`j`) にちょうど落ちる。長さは 20 列ぶん。
- **USB が左を向いた、実物のピンアウト図と同じ向き**になる
  (下の列が左から GP0, GP1, GND, …、上の列が左から VBUS, VSYS, GND, …)。
  `c` 行に書いても `h` 行に書いても同じ。USB を右に向けるときは `r180`。
  USB は必ずピン 1 の側の端に描く。
...
## 足に名前のある DIP 型 (リレー・フォトカプラ・7 セグ)

リレー・フォトカプラ・7 セグは、**DIP の足の位置のうち、足のある所に名前が付いた物**
として置く。書き方は DIP と同じ (胴の左端の列の穴 1 つ) で、足は名前で呼べる。
足の並びは実物のデータシートどおり (表は 3 つのフェンスで同じ)。

```yaml
parts:
  K1: relay @ f10            # G5V-2 (姿を書かなければ表の最初)
  U1: photocoupler/pc817 @ f20
  DS1: seg7 @ i5
wires:
  - g13 -- +b13 red          # K1.COM1 の列 (13 列の下のブロック)
```
...
- **列の間は種類で決まる。** リレーとフォトカプラは DIP と同じ e 行か f 行、
  7 セグは 6 穴離れた行の組 (b↔f、c↔g、d↔h、e↔i) に置く。届かない行に書くと、
  置ける行を添えて断る
- 足は DIP と同じく**実物を上から見た並び** (切り欠きを左にして 1 番が左下)。
  組になる行のどちら (7 セグなら `b` でも `f` でも) に書いても同じ向きに挿さる
- 何も書かなければ図と部品リストに品名 (`G5V-2`) が出る。7 セグは面 (8 の字) を描く
- ID の接頭辞は `K` (リレー)・`U` (フォトカプラ)・`DS` (7 セグ)
- 図は [例 14](../examples/14-named-chips.md)

## 部品の向き (r180)

**アンカー 1 つで置く形 (`dipN` / `sipN` / マイコンボード) だけ**が向きの語を持つ。
足を並べて書く部品 (2 本足・3 本足) の向きは**穴の順そのもの**なので、語は要らない。
...
parts:
  U1: dip8 @ e5 NJM4556A
  U2: dip8 @ e15 r180 NJM4556A
```

![図20 同じ穴に、向きだけ変えて挿す](out/01-syntax-20.svg)

切り欠き (○) は**必ず 1 番ピンの側**に出る。図のとおりに挿せば実物も同じ向きになる。

書けない向きは、なぜ書けないかを言って断る。

| 語 | 断る理由 |
| --- | --- |
| `r90` / `r270` | **溝をまたぐ物理そのもの。** DIP の 2 列は e 行と f 行に固定されていて、90 度回すと 2 列が同じ列に重なる。1 列ヘッダも、縦にすると足が全部同じ 5 穴に入って短絡する |
| `mirror` | **実物は裏返して挿せない。** 裏返した形は IC を裏から見た並びで、どう挿しても作れない。向きを変えるなら `r180` |

タクトスイッチ (`button`) はアンカー 1 つで置く形だが、**対称なので向きを持たない** —
回しても同じ穴どうしがつながるだけになる。

マップ形式では `turn: r180` と書く。読み方も断り方も 1 行記法と同じ。
...
![図21 ボード外の機器をつなぐ](out/01-syntax-21.svg)
...
- 機器から板への線は、**横の区間を機器の帯と板の間 (穴の無い帯) で済ませ**、
  板へは縦に降りる。レールをまたいで降りるときは**列と列の間**を降り、
  穴の半行手前で横に入って挿す。板の中の横レーン (レールの縁やレールの上) を
  横に走ると、レールにつながって見えるため。
...
- **差し込み口は溝の反対側を向く** — 上のブロックなら上、下のブロックなら下。
  ケーブルは板の外へ出すのが普通で、そちらの縁が近い。板からはみ出すぶんは
  画布を伸ばす (切って描くと、切れたことに気づけない)。
- 姿は `male` (差し込み。金物が変換基板の縁から出る) と `female` (受け口、既定)。
  Type-A の差し込みは天板の 2 つの角窓、受け口は口の暗がりが目印。
- 回路図フェンス・ユニバーサル基板フェンスとも**同じ種類名・同じ足の名前**で書ける。
...
**上下のレールを渡す線のように、同じ列の 2 穴を長く縦に結ぶ線は、列の間を通す**
(`-b20 -- -t20`)。列の真上をまっすぐ引くと、途中の 10 穴に挿さった部品の足や
配線の端が線に隠れて、その列につながって見えるため。穴からは半行だけ縦に出て、
行と行の間で横に振ってから縦に走る。同じ列で 2 本渡すときは別々の隙間を通る。
同じブロックの中 (`a5 -- e5`) と、レールから隣のブロックへの短いホップ
(`+t5 -- a5`) は今までどおりまっすぐ引く (またぐのは同じ列の穴か、
もう 1 本のレールの穴 1 つだけ)。

**迂回ヒントの最後の横の区間も行と行の間を通す** (`AD.1+ -- b5 [h10]`)。
降りきってから穴の行の上を横に走ると、その行のジャンパに見えるため。
