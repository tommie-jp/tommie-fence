...
として置く。書き方は DIP と同じ (1 番ピンの穴 1 つ) で、足は名前で呼べる。
...
- **列の間は種類で決まる。** リレーとフォトカプラは DIP と同じ e 行か f 行、
  ...
  置ける行を添えて断る
- 1 番ピンを**下のブロック** (f 行など) に置くと、切り欠きを左にした実物を上から
  見た並び (1 番が左下) と同じになる
...
| `mirror` | **アンカーの行がもう言っている。** `dip8 @ e5` を裏返した形は `dip8 @ f5` そのもの。語を足すと、同じ置き方を 2 通りで書けるようになる |
