...
describe('collectProblems', () => {
  test('puts an unreadable line of any fence on its Markdown line', () => {
    ...
    expect(problems.filter((one) => one.kind === 'error')).toEqual([
      { language: 'perfboard', line: 12, kind: 'error', message: expect.stringContaining('resistr') },
    ]);
    expect(problems.every((one) => one.language === 'perfboard')).toBe(true);
    // perfboard は「読めないので ERC を掛けていない」とも言う (行を持たないので開き記号の行)。
    expect(problems).toContainEqual({ language: 'perfboard', line: 9, kind: 'notice', message: expect.stringContaining('ERC') });
  });
  ...
  test('adds ERC only when asked', () => {
    ...
    expect(loud.every((one) => one.kind === 'erc' && one.language === 'perfboard')).toBe(true);
  });
