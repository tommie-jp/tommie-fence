...
const LANGUAGE = 'copper';

/**
 * ```copper フェンスを図に差し替える markdown-it プラグイン。
 * VS Code のプレビューはここが返した HTML をサニタイズしないので、
 * 文字列の組み立てはすべて core 側のエスケープを通ったものだけを使う。
 */
export function copperPlugin(md: MarkdownIt): MarkdownIt {
  ...
  md.renderer.rules.fence = (tokens, index, options, env, self) => {
    ...
    const { svg, errorHtml } = renderCopper(token.content, { offset });
