...
const rowsHtml = (error: FenceError): string => {
  ...
  return `<pre class="copper-error-source">${escapeMarkup(rows.join('\n'))}</pre>`;
};

const itemHtml = (error: FenceError): string => {
  const kind = error.notice === true ? ' copper-notice' : '';
  return `<li class="copper-error-item${kind}">`
    + `<span class="copper-error-line">${escapeMarkup(errorLine(error))}</span>`
    + `${rowsHtml(error)}</li>`;
};
...
function listHtml(errors: readonly FenceError[]): string {
  ...
  const rest = errors.length > MAX_SHOWN
    ? `<li class="copper-error-item">ほかに ${errors.length - MAX_SHOWN} 件</li>`
    : '';
  return `<ul class="copper-error-list">${shown}${rest}</ul>`;
}

/** 図は描けたが一部が読めなかったときに、図の下へ貼る帯。 */
export const renderErrorBanner = (errors: readonly FenceError[]): string =>
  errors.length === 0 ? '' : `<div class="copper-errors">${listHtml(errors)}</div>`;
