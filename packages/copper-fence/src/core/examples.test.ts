...
import { extractCopperFences, renderCopper } from './index.ts';
...
describe('examples', () => {
  ...
  test.each(markdownFiles)('%s renders to the drawing committed in examples/out', (name) => {
    const fences = extractCopperFences(read(name));
    expect(fences.length).toBeGreaterThan(0);

    for (const [index, fence] of fences.entries()) {
      const { svg } = renderCopper(fence.source);
      expect(svg).not.toBe('');
  ...
  test.each(markdownFiles)('%s reads without an error', (name) => {
    for (const fence of extractCopperFences(read(name))) {
      // お知らせ (ERC) はわざと出している例があるので見ない。
      expect(renderCopper(fence.source).errors).toEqual([]);
    }
  });

  test.each(markdownFiles)('%s pastes a drawing after every fence', (name) => {
    const text = read(name);
    const fences = extractCopperFences(text);
...
describe('docs/01-syntax.md', () => {
  ...
  test('renders to the drawings committed in docs/out', () => {
    for (const [index, fence] of fences.entries()) {
      const { svg, errors } = renderCopper(fence.source);
...
describe('errors/', () => {
  ...
  test.each(errorFiles)('%s is written wrong on purpose, and says so', (name) => {
    const fences = extractCopperFences(read(`errors/${name}`));
    expect(fences.length).toBeGreaterThan(0);

    for (const fence of fences) {
      // **どのフェンスも必ず何か言う。** 直ったのに errors/ に残っていると、
      // 「読めない例」として貼ってあるものが黙って読めるようになる。
      expect(renderCopper(fence.source).errors.length).toBeGreaterThan(0);
    }
  });
});
