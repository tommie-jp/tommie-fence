...
import { errorText, extractCopperFences, renderCopper } from '../core/index.ts';
...
function jobsFor(path: string, outDir: string | null): Job[] {
  ...
  const fences = extractCopperFences(source);
  ...
  try {
    ...
    for (const target of targets.flatMap(collectFiles)) {
      for (const job of jobsFor(target, outDir)) {
        const result = renderCopper(job.source, { offset: job.offset });
