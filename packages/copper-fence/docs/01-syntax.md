...
**裏から見た図は既定で出す。** 板を縦の軸で裏返して見た姿 (左右が入れ替わる) で、
裏に地のある板は銅のベタ、無い板は基材のまま、via は抜けた穴、切り欠きは溝、
端面 SMA は裏の腕だけを描く。表の銅は薄く透かして出す (裏で穴を開ける位置を
表と合わせるため)。銅張り基板は裏の地でネットが決まるので、perfboard の半田面
(既定で出さない) と逆にしてある。マップ (掴む図) には出さない。
