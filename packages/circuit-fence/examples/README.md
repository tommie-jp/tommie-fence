...
どの図にも `title:` で内容の題を付けてある。**番号 (回路図NN) は付けない** —
番号は [docs/syntax.md](../docs/syntax.md) の図だけが持つ。例の図は syntax.md に
埋め込まれるので、こちらに番号を焼き込むと syntax.md の表示順と食い違って
見える。番号が紛れ込んでいないかはテストで見張っている。
