...
どの図にも `title:` で題を付けてある。**[docs/syntax.md](../docs/syntax.md) に
貼ってある図には `図NN` の番号も付く** — 文章から「図12 を直して」と指せる
ようにするため。番号は syntax.md に出てくる順で数えるので、例の側のファイル順とは
揃わない (貼られる図の番号は、それを作っているこちらのフェンスに書く)。
syntax.md に出てこない図には番号を付けない。番号がずれていないかはテストで
見張っている。
