...
[docs/01-syntax.md](../docs/01-syntax.md) はフェンスを直に書き、その直後に
**自分のフェンスを描いた図** (`docs/out`、作り直しは `npm run docs`) を貼ってある。
こちらの図は流用しない — 図には題番号が焼き込まれていて、examples 側の番号を
名乗ると向こうの体系が崩れる。同じ図が両方にあるので、**題が同じなら中身も同じ**
であることをテストで見張っている — 片方だけ直すと、同じ名前の図が
2 通りの姿で出てしまう。
