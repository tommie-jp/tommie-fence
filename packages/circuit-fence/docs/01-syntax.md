...
**ブザーと SMA コネクタは、別の記号で描いている。** circuitikz 1.0 に
ブザーの記号 (`buzzer` / `bell` / `piezo` / `sounder`) も同軸コネクタの記号
(`coax` / `plug` / `socket` / `jack`) も無いことを実機で確かめた。
ブザーは同じ音を出す仲間の**スピーカーの記号**、SMA は記号の無いコネクタの
慣習どおり**箱に名前** (`J1`) で描く。
