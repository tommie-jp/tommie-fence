...
**欄で直す。** 部品を選ぶとマップの下に欄が出る (名前・種類・値・ラベル)。
`Enter` か欄を離れたときに当たり、名前を変えると配線の足と注釈の指し先も
一緒に書き換わる。

**部品を置く。** 頭のパレットで種類を選ぶと「置く」道具になり、2 端子は交点から
交点へドラッグ、ほかは交点をクリックで置ける。ID は接頭辞ごとに最小の未使用番号
(`R1` の次は `R2`)。図に名前が出る `port` / `vcc` / `vee` だけは名前を訊く。
