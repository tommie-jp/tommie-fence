...
- `Circuit Fence: 部品を動かす (マップ)` — 升目のパネルを横に開き、**ドラッグして**
  置きたい交点で放す (クリックは選ぶだけで動かない)。頭の切り替えで、掴む物が
  部品から節点へ変わる。升目には**部品の形と配線**が出る (記号の正確さは図の
  ほうが正しい。マップはどれがどこにいるかを掴むための似顔絵)。
...
接続を**動かしたあとに**出す。戻すのはマップの「元に戻す」か `Ctrl+Z`
(パネルにフォーカスがあっても効くように、パネルが履歴を覚えている)。
