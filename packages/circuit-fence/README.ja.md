...
- `Circuit Fence: 部品を動かす (マップ)` — 升目のパネルを横に開き、掴んで
  置きたい交点で放す。頭の切り替えで、掴む物が部品から節点へ変わる。
  升目には**部品の形と配線**が出る (記号の正確さは図のほうが正しい。
  マップはどれがどこにいるかを掴むための似顔絵)。
  マップとエディタは**互いに指し合う** — 掴んだものは書いてある場所が光り、
  カーソルが指しているものはマップで光る
